...
class FFNN(nn.Module):
    ...
    def train(self, x, y, verbose=False):
        ...
        while (n < self.epochs):
            ...
            cost.backward()
            optimizer.step()
            if n % (self.epochs/100) == 0:
                if verbose: print(cost)
                costs.append(cost)
            if n % (self.epochs/10) == 0 and verbose == True:
                _metrics = util.getClassificationMetrics(y_pred, y_true.cpu().data.numpy(), probability=True, verbose=False)
                f1, acc, recall, precision = _metrics.values()
                if verbose: print(f"Metrics:\nF1 = {f1:0.3f}\nAccuracy = {acc:0.3f}\nRecall = {recall:.3f}\nPrecision = {precision:.3f}\n")
            n += 1
        if verbose: print(cost)
