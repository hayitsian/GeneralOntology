...
def getClassificationMetrics(predictions, labels, probability=False, threshold=0.5, upperVal=1, lowerVal=0, verbose=True):
    ...
    if probability: y_pred = np.where(probs >= threshold, upperVal, lowerVal)
    else: y_pred = probs

    y_true = labels

    f1_score = metrics.f1_score(y_true=y_true, y_pred=y_pred, average='micro')
    # roc_auc = metrics.roc_auc_score(y_true, y_pred, multi_class='ovo', average = 'micro')
    ...
    if not probability: 
        ...
        _metrics['confusion matrix'] = confusion_matrix
    
    if verbose: print(f"F1: {f1_score}\tAccuracy: {accuracy}\n"# AUC: {roc_auc}\tAccuracy: {accuracy}\n"
